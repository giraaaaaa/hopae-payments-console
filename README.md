...
## 와이어프레임 대비 결정 (바꿈 / 더함 / 유지)

과제 안내대로 와이어프레임은 "정답 디자인"이 아니라 참고 자료로 받아들이고,
아래처럼 판단했습니다. (근거는 각 항목의 §링크 참고.)

**바꿈** — 있지만 다른 방식이 낫다고 판단

| 와이어프레임 | 구현 | 이유 |
| --- | --- | --- |
| "Status: All ▾" 드롭다운 | 라이브 카운트 언더라인 탭(데스크톱) / 바텀시트(모바일) | 드롭다운은 상태별 건수가 안 보이고 조작 2탭. 탭은 분포가 한눈에, 바텀시트는 모바일 네이티브 문법 |
| Amount · Currency 별도 컬럼 | 통합 표기 `$1,250.00 USD` | 통화 기호·코드를 분리하면 스캔이 두 번 — 금액은 한 덩어리로 읽음 |
| Created "2m ago"만 | 상대시간 + 절대시각 2줄 상시 | 대사(reconciliation)에서 정확한 시각은 호버에 숨길 정보가 아님 |
| 전 행 Refund 버튼 | `succeeded` 행에만 노출 | 환불 불가한 행에 버튼을 주고 거절하기보다 액션 자체를 제거 |

**더함** — 없지만 필요하다고 판단

| 추가 | 이유 |
| --- | --- |
| sticky "↑ N new" pill | 준실시간의 핵심 답 — 목록을 밀지 않고 신규 통지, 스크롤해도 따라옴 |
| 상태 변화 1.6초 하이라이트 | `pending` 확정을 지켜보는 시나리오의 변화 인지 |
| 실패 사유 상시 표시 | `failed`만 보고 원인을 모르면 대응 불가 |
| Payment 컬럼(브랜드+뒤4자리) | 고객 문의 시 카드 특정 |
| Production 환불 체크박스 게이트 | 환경별 UX 차등 — 실돈에만 마찰 추가 (§4-2) |
| 갱신 실패 배너 + 자가 복구 | "로드 실패"와 "갱신 실패"를 구분 (§5) |
| 하단 탭바 / 토스트 | 모바일 주 내비게이션 / 행동 결과 피드백 |

**유지** — 그대로가 옳다고 판단: 노란 "test data" 상시 배너, 환불 확인 모달
구조(금액 명시 + "This can't be undone"), 장식 네비(Customers/Payouts/
Developers, 비활성 처리로 오해 방지). **뺀 것: 없음** — 와이어프레임 4장의
핵심 요소는 전부 구현.
...
### 알고 있으면서 이번엔 남겨둔 것 (인지한 한계)

"몰라서 못 한 것"과 구분하기 위해 명시합니다.

- **모달 포커스 트랩 없음** — 환불 모달은 Esc 닫기·포커스 이동·백드롭 클릭은
  되지만, Tab이 모달 밖으로 샐 수 있습니다. 접근성 완성도를 높이려면 focus
  trap(또는 `<dialog>` 요소)이 필요합니다.
- **멀티탭 로그아웃 비동기화** — 토큰은 `localStorage` 공유지만 `storage`
  이벤트를 듣지 않아, 한 탭에서 로그아웃해도 다른 탭은 다음 401까지 살아
  있습니다. `storage` 리스너로 즉시 동기화 가능.
- **"Load more" 실패의 개별 피드백** — 다음 페이지 로드가 실패하면 조용히
  멈춥니다(전체 목록은 유지). 인라인 "다시 시도" 행을 붙이는 게 낫습니다.
- **서버 DTO ↔ 웹 타입 수동 복제** — `TransactionRow` 등을 양쪽에 따로
  정의해 drift 위험이 있습니다. 공유 패키지나 OpenAPI 코드젠으로 단일
  소스화하는 게 정석입니다.
- **`useTransactionList` 훅 자체는 미테스트** — 순수 로직(서버 쿼리·환불,
  금액 포맷)은 유닛 테스트 19개로 덮었지만, 가장 복잡한 이 훅은 브라우저
  수동 검증만 했습니다. 위 E2E와 함께 보강 대상.
